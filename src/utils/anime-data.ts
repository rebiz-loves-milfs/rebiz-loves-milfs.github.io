...
export function getAnimeSourceConfigs(siteConfig?: SiteConfig): Record<string, AnimeSourceConfig> {
	return {
		local: {
			type: "local",
			data: localAnimeList,
		...
		anilist: {
			type: "json",
			filename: "anilist-data.json",
			fetchOnDev: siteConfig?.anilist?.fetchOnDev ?? true,
			emptyDescription: i18n(I18nKey.animeEmptyAniList),
		},
	};
}

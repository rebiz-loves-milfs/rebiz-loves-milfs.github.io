 ...
 */
export function glitchHeading(glitchIvRef: {
  current: ReturnType<typeof setInterval> | null;
}): HTMLElement | null {
  ...
  glitchIvRef.current = setInterval(() => {
    ...
    el.style.cssText =
      orig +
      `;transform:translate(${dx}px,${dy}px);text-shadow:${dx}px 0 oklch(0.6 0.3 ${(h + 180) % 360}),${-dx}px 0 oklch(0.6 0.3 ${(h + 60) % 360});`;
    if (n > 9) {
      clearInterval(glitchIvRef.current!);
      glitchIvRef.current = null;
...
export function stickyNote(): HTMLElement | null {
  const c = pick(
    visEls(".card, .post-item, .archive-row, .profile"),
  ) as HTMLElement & { __nx7sticky?: boolean };
  if (!c || c.__nx7sticky) return null;
  c.__nx7sticky = true;
  const lines = [
    "→ read this one",
    "rebiz wrote this past midnight",
    "good vibes here ♡",
    "underrated",
    "TODO: reread",
    "★ favourite",
...
export function scribbleUnder(): HTMLElement | null {
  const link = pick(
    visEls("a.nav-link, .post-body a, .post-body h2, .archive-row"),
  );
  if (!link) return null;
  ...
  path.setAttribute(
    "d",
    "M 0 6 C 8 1,16 11,24 6 S 40 1,48 6 S 64 11,72 6 S 88 1,100 6",
  );
...
export function markTarget(
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const size = 24 + i * 18;
    ring.style.cssText = `position:fixed;left:${cx - size / 2}px;top:${cy - size / 2}px;width:${size}px;height:${size}px;border-radius:50%;border:${1.5 - i * 0.3}px solid oklch(0.85 0.22 ${h}/${1 - i * 0.25});box-shadow:0 0 ${8 + i * 4}px oklch(0.75 0.22 ${h}/${0.6 - i * 0.15});z-index:9998;pointer-events:none;opacity:0;transform:scale(2.5);transition:transform ${0.35 + i * 0.12}s cubic-bezier(.2,.8,.4,1) ${i * 0.06}s, opacity .2s ${i * 0.06}s;`;
    ...
    setTimeout(
      () => {
        ...
        ring.style.transition = "opacity .4s";
  ...
  if (el) {
    ...
    setTimeout(() => {
      dot.style.opacity = "0";
      dot.style.transition = "opacity .4s";
...
export function sparkle(x: number, y: number, count = 6): void {
  ...
  for (let i = 0; i < count; i++) {
    ...
    requestAnimationFrame(() => {
      sp.style.transform = `translate(${Math.cos(ang) * dist}px,${Math.sin(ang) * dist}px) scale(.3)`;
      sp.style.opacity = "0";
    });
...
export function spawnGhosts(elRef: HTMLElement | null): void {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    setTimeout(
      () => {
        g.style.transform = `translate(${dx}px,${dy}px) scale(${0.5 + Math.random() * 0.7}) rotate(${(Math.random() - 0.5) * 20}deg)`;
        g.style.opacity = "0";
      },
      i * 80 + 40,
    );
...
export function spawnImpact(x: number, y: number): void {
  ...
  requestAnimationFrame(() => {
    ...
    burst.style.transform = "scale(1.2) rotate(8deg)";
  });
  setTimeout(() => {
    burst.style.opacity = "0";
    burst.style.transform = "scale(.9) rotate(15deg) translateY(-8px)";
  ...
  for (let i = 0; i < 6; i++) {
    ...
    requestAnimationFrame(() => {
      sp.style.transform = `translate(${Math.cos(ang) * dist}px,${Math.sin(ang) * dist}px) scale(.4)`;
      sp.style.opacity = "0";
    });
...
export function slapLabel(
  target: HTMLElement & { __nx7slap?: boolean },
  text: string,
): void {
  ...
  requestAnimationFrame(() =>
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        label.style.transform = `translateX(-50%) translateY(0) rotate(${t}deg) scale(1)`;
      }, 350);
    }),
  );
  setTimeout(() => {

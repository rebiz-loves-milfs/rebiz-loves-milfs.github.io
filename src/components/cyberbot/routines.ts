...
import {
  pick, getHue, visEls,
  highlightParagraph, stickyNote, scribbleUnder,
  screenRipple, markTarget, sparkle, spawnGhosts, spawnImpact, slapLabel,
} from './effects';
...
export interface BotContext {
  ...
  speak:  (text: string, dur?: number, action?: { route: string; label: string } | null) => void;
...
export function createRoutines(ctx: BotContext) {
  ...
  async function r_climb() {
    const el = pick(visEls('.card,.post-item,.anime-cover')); if (!el) return;
    const r = el.getBoundingClientRect();
    await moveTo(r.left + r.width / 2 - 50, r.top - 110);
    ctx.setBotState('climbing'); speak(pick(D.climb), 2800); await pause(2400); ctx.setBotState('idle');
  }

  async function r_knock() {
    await moveTo(window.innerWidth / 2 - 50, 100);
    ctx.setBotState('knocking'); speak(pick(D.knock), 2200);
  ...
  async function r_read() {
    ...
    let i = 0;
    const sc = setInterval(() => { i++; ctx.setEye({ x: Math.sin(i * .5) * 2.8, y: 0 }); }, 90);
    await pause(2800);
    clearInterval(sc);
    ctx.setEye({ x: 0, y: 0 });
  ...
  async function r_glitch_text() {
    ...
    const el = glitchHeading(ctx.glitchIvRef); if (!el) return;
    const r = el.getBoundingClientRect();
  ...
  async function r_stretch() { ctx.setBotState('stretching'); speak(pick(D.stretch), 2000); await pause(1800); ctx.setBotState('idle'); }
  async function r_glitch()  { ctx.setBotState('glitching');  speak(pick(D.glitch),  2000); spawnGhosts(ctx.getElRef()); await pause(900); ctx.setBotState('idle'); }

  async function r_wander() {
    await moveTo(60 + Math.random() * (window.innerWidth - 180), 80 + Math.random() * (window.innerHeight - 220));
  ...
  async function r_suggest() {
    ...
    if (link) {
      ...
      for (let i = 0; i < 3; i++) { ctx.setFacing(i % 2 ? 1 : -1); await pause(220); }
      ctx.setEyeMode('closed'); await pause(220);
      ctx.setEyeMode('smirk');  await pause(1400);
  ...
  async function r_chase() {
    ctx.setBotState('chasing'); speak(pick(D.chase), 1600);
    for (let i = 0; i < 4; i++) { const c=ctx.getCursor(); await moveTo(c.x-50, c.y-50, {speed:4}); await pause(150); }
    ctx.setBotState('idle');
  }
  ...
  async function r_tour() {
    ...
    const next = pick(['anime','archive','about','home'].filter(r => r !== cur));
    ctx.setBotState('touring'); speak('INITIATING TOUR. NAVIGATING TO '+next.toUpperCase()+'.', 3000);
    ...
    await pause(1800); window.location.href = '/'+next;
  }
  ...
  async function r_nod() {
    ...
    for (let i = 0; i < 4; i++) { const p0=ctx.getPos(); ctx.setPos({x:p0.x,y:p0.y-8}); await pause(180); const p1=ctx.getPos(); ctx.setPos({x:p1.x,y:p1.y+8}); await pause(180); }
  }

  async function r_panic() {
    ctx.setBotState('panicking'); ctx.setEyeMode('surprised');
    speak('WAIT. WAIT. WAIT. THIS IS FINE.', 2200);
    for (let i=0;i<5;i++) { await moveTo(40+Math.random()*(window.innerWidth-180), 80+Math.random()*(window.innerHeight-220), {speed:9}); await pause(80); }
    ctx.setEyeMode('tired'); speak('...FINE. THAT WAS PLANNED.', 2000); await pause(800);
    ctx.setEyeMode('normal'); ctx.setBotState('idle');
  }
  ...
  async function r_investigate() {
    ...
    let n = 0;
    const sc = setInterval(() => { n++; ctx.setEye({ x: Math.sin(n * .8) * 3.5, y: Math.cos(n * .4) * .8 }); }, 60);
    await pause(1800);
    clearInterval(sc);
    ctx.setEye({ x: 0, y: 0 });
    ctx.setEyeMode('normal');
    speak(['ANALYSIS COMPLETE. VERDICT: INTERESTING.', 'DETECTED. LOGGED. EVALUATED.', 'I HAVE QUESTIONS. THEY ARE RHETORICAL.', 'MY SENSORS ARE INTRIGUED.', 'CLASSIFIED AS: NOTABLE.'][Math.floor(Math.random() * 5)], 2600);
    ctx.setBotState('idle');
  }
  ...
  async function r_sit() {
    ...
    ctx.setEyeMode('smirk');
    for (let i = 0; i < 5; i++) {
      const p0 = ctx.getPos(); ctx.setPos({ x: p0.x, y: p0.y - 4 }); await pause(200);
      const p1 = ctx.getPos(); ctx.setPos({ x: p1.x, y: p1.y + 4 }); await pause(200);
    }
  ...
  async function r_write_screen() {
    ...
    const h   = getHue();
    const txt = pick(['← READ THIS', '★ NOTED', '!! IMPORTANT', '→ CLICK ME', '♡ FAVE']);
    ...
    requestAnimationFrame(() => { lbl.style.opacity = '1'; lbl.style.transform = `rotate(${rot}deg) scale(1)`; });
    ...
    await pause(3800);
    lbl.style.opacity = '0'; lbl.style.transform = `rotate(${rot}deg) scale(0.6)`;
    svg.style.opacity = '0'; (svg.style as unknown as Record<string, string>).transition = 'opacity .35s';
    setTimeout(() => { lbl.remove(); svg.remove(); }, 380);
  }
  ...
  async function r_matrix() {
    ctx.setBotState('glitching'); ctx.setEyeMode('wide');
    speak(pick(D.hacker), 1800); spawnGhosts(ctx.getElRef()); await pause(900);
    (window as Window & { __nx7_triggerPeel?: () => void }).__nx7_triggerPeel?.();
    speak(pick(D.matrix), 2800); await pause(1800);
    ctx.setBotState('idle'); ctx.setEyeMode('normal');
  }
  ...
  function pickRoutine(): () => Promise<void> {
    ...
    for (const r of ALL) { if ((n -= r.w) <= 0) return r.fn; }
    return r_wander;
  }

...
export interface SiteConfig {
	...
	anilist?: {
		username?: string;
		fetchOnDev?: boolean;
		url?: string; // AniList profile URL
	};

	anime?: {
		mode?: "anilist" | "local" | "bangumi" | "bilibili";
	};
...
export interface MusicPlayerConfig {
	...
	mode: "local" | "youtube" | "youtube-iframe";
	youtubePlaylistId?: string;
	youtubeVideoId?: string; // Single YouTube video ID
	pipedApiUrl?: string; // Only used for youtube mode (Piped API)
	customCoverUrl?: string; // Custom cover image URL (overrides default)
}

export interface LastFmConfig {
	enable: boolean;
	username: string; // Last.fm username
	apiKey: string; // Last.fm API key (safe for client-side)
	recentTracksLimit?: number; // Number of recent tracks to display (default: 5)
...
export interface DiscordStatusConfig {
	...
	useWebSocket?: boolean; // Use WebSocket instead of polling (default: true)
}
...
export interface GuestbookConfig {
	...
	supabaseUrl: string;
	supabaseKey: string;
	adminPassword: string; // UI-only gate — stored in config, not a true secret
...
export type WidgetComponentType =
	...
	| "site-stats" // 站点统计组件
	| "calendar" // 日历组件
	| "lastfm" // Last.fm widget
	| "discord-status" // Discord status widget
	| "custom";
